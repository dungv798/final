<?php

namespace App\Http\Controllers;

use GuzzleHttp\Client;
use App\Models\Cart;
use App\Models\Orders;
use App\Models\Products;
use App\Models\User;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Http;
use Google\Cloud\Core\ExponentialBackoff;
use Google\Cloud\DistanceMatrix\V1\DistanceMatrixServiceClient;
use Google\Cloud\DistanceMatrix\V1\DistanceMatrixRequest;
use Google\Protobuf\StringValue;
use Google\Type\LatLng;
use GoogleMaps\GoogleMaps;

class OrderManager extends Controller
{
    function newOrders()
    {
        $orders = Orders::where("status", "open")->get();
        $orders = json_decode(json_encode($orders));
        $delivery_boys = User::where("type", "delivery")->get();
        $products = Products::get();
        foreach ($orders as $key => $order) {
            $order_item_ids = json_decode($order->items);
            foreach ($order_item_ids as $key2 => $order_item) { //pizza = 2 and fried rice =1
                foreach ($products as $product) {
                    if ($order_item->item_id == $product->id) {
                        $orders[$key]->item_details[$key2] = $product;
                    }
                }
            }
        }
        return view("dashboard", compact("orders", "delivery_boys"));
    }

    function assignOrder(Request $request)
    {
        $order = Orders::where("id", $request->order_id)->first();
        $order->delivery_boy_email = $request->delivery_boy_email;
        $order->status = "assigned";
        if ($order->save()) {
            return redirect(route("dashboard"))
                ->with("success", "Order assigned successfully");
        }
        return redirect(route("dashboard"))
            ->with("error", "Failed to assign Order");
    }

    function listOrders()
    {
        $orders = Orders::orderBy("id", "DESC")->get();
        $orders = json_decode(json_encode($orders));
        $products = Products::get();
        foreach ($orders as $key => $order) {
            $order_item_ids = json_decode($order->items);
            foreach ($order_item_ids as $key2 => $order_item) { //pizza = 2 and fried rice =1
                foreach ($products as $product) {
                    if ($order_item->item_id == $product->id) {
                        $orders[$key]->item_details[$key2] = $product;
                    }
                }
            }
        }
        return view("order", compact("orders"));
    }

    function addToCart(Request $request)
    {
        $cart = new Cart();
        $cart->item_id = $request->item_id;
        $cart->user_email = $request->user_email;
        if ($cart->save()) {
            return "success";
        }
        return "failed";
    }

    function removeFromCart(Request $request)
    {
        $cart = Cart::where("item_id", $request->item_id)
            ->where("user_email", $request->user_email)->first();
        if ($cart == null) {
            return "failed";
        }
        if ($cart->delete()) {
            return "success";
        }

        return "failed";
    }

    function getCart(Request $request)
    {
        $item_id = array();
        $count_items = DB::select(
            "SELECT item_id, COUNT(item_id) as num_item from cart
                where user_email = '" . $request->user_email . "'
                GROUP BY item_id");
    
        foreach ($count_items as $key => $item) {
            $item_id[$key] = $item->item_id;
        }
    
        $user = User::where("email", $request->user_email)->first();
        $distanceAndDuration = $this->calculateEstimatedTime($user);
        $deliveryCost = 0;
    
        if (isset($distanceAndDuration['distance'])) {
            $deliveryCost = $this->calculateDeliveryCost($distanceAndDuration['distance']);
        }
    
        $products = Products::whereIn('id', $item_id)->get();
        foreach ($count_items as $item) {
            foreach ($products as $key => $product) {
                if ($item->item_id == $product->id) {
                    $products[$key]->numItem = $item->num_item;
                }
            }
        }
    
        $data = array(
            "cart" => json_decode($products),
            "delivery_cost" => $deliveryCost
        );
    
        if (isset($distanceAndDuration['duration'])) {
            $data["duration"] = $distanceAndDuration['duration'];
        }
        if (isset($distanceAndDuration['distance'])) {
            $data["distance"] = $distanceAndDuration['distance'];
        }
    
        if (isset($distanceAndDuration['error'])) {
            $data["error"] = $distanceAndDuration['error'];
        }
    
        return $data;
    }

    function confirmCart(Request $request)
    {
        $cart = Cart::select("item_id")->where("user_email", $request->user_email)->get();
        if (empty($cart->first())) {
            return "failed";
        }
        $user = User::where("email", $request->user_email)->first();
        $order = new Orders();
        $order->customer_email = $request->user_email;
        $order->items = $cart;
        $order->status = "open";
        $order->destination_address = $user->destination_address;
        $order->destination_lat = $user->destination_lat;
        $order->destination_lon = $user->destination_lon;
        if ($order->save()) {
            if ($this->clearCart($request) == "success") {
                return "success";
            }
        }

        return "failed";
    }

    function clearCart(Request $request)
    {
        if (Cart::where("user_email", $request->user_email)->delete()) {
            return "success";
        }

        return "failed";
    }

    



    function calculateEstimatedTime($user)
{
    // Định nghĩa tọa độ ban đầu từ cửa hàng
    $origin_lat = 10.146155908876807; 
    $origin_lon = 11.146155908876807;
    $apiKey = 'AIzaSyBI9DABKxmVc96cnJVQ47rT0aSTlAocYqc';  // Đổi 'API_KEY_HERE' thành API key thực của bạn

    $client = new Client();
    $url = "https://maps.googleapis.com/maps/api/distancematrix/json?";

    try {
        // Thực hiện yêu cầu lên Google Maps API
        $response = $client->get($url, [
            'query' => [
                'units' => 'metric',
                'origins' => "$origin_lat,$user->destination_lat",
                'destinations' => "$origin_lon,$user->destination_lon",
                'mode' => 'driving',
                'key' => $apiKey
            ]
        ]);

        // Giải mã phản hồi
        $data = json_decode($response->getBody(), true);

        // Kiểm tra dữ liệu phản hồi
        if (isset($data['status']) && $data['status'] === 'OK' && isset($data['rows'][0]['elements'][0]['status']) && $data['rows'][0]['elements'][0]['status'] == 'OK') {
            $element = $data['rows'][0]['elements'][0];
            
            $dist_dur = [];
            $dist_dur['distance'] = isset($element['distance']['value']) ? $element['distance']['value'] : 0;  // meters
            $dist_dur['duration'] = isset($element['duration']['value']) ? $element['duration']['value'] : 0;  // seconds

            return $dist_dur;
        } else {
            // Trả về thông báo lỗi và phản hồi từ API nếu có lỗi
            return [
                'error' => 'Không thể lấy dữ liệu từ Google Maps API',
                'response' => $data
            ];
        }
    } catch (\Exception $e) {
        // Trả về lỗi nếu có vấn đề khi gửi yêu cầu
        return [
            'error' => 'Lỗi khi gửi yêu cầu tới Google Maps API: ' . $e->getMessage()
        ];
    }
}

    
function calculateDeliveryCost($distanceInMeters)
{
    $ratePerMeter = 0.005; // Ví dụ: 0.005 USD cho mỗi mét, bạn có thể thay đổi giá này.
    return $distanceInMeters * $ratePerMeter;
}


    function getOrders(Request $request){
        $orders = Orders::where("customer_email", $request->email)->orderBy("id", "DESC")->get();
        $orders = json_decode(json_encode($orders));
        $products = Products::get();
        foreach ($orders as $key => $order) {
            $order_item_ids = json_decode($order->items);
            foreach ($order_item_ids as $key2 => $order_item) { //pizza = 2 and fried rice =1
                foreach ($products as $product) {
                    if ($order_item->item_id == $product->id) {
                        $orders[$key]->item_details[$key2] = $product;
                    }
                }
            }
        }

        return $orders;
    }

}
